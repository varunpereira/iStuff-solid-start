...
export default () => {
	...
	return d(
		{style: () => "c_white w_full r_full mr-[1rem] z_fit"},
		i({
			type: () => "text",
			value: () => form_data().search,
			input: (e) => form_data({...form_data(), search: e.target.value}),
			holder: () => "search from millions of products...",
			style: () => "tc_black r_full px-[.5rem] w_full h_full o_null",
		}),
		b(
			{click: form_submit},
			search_icon({
				style: () => "z_put c_white right-[.4rem] top-[.2rem] tc_black w-[1.5rem] h-[1.5rem]",
			}),
		),
	)
}
